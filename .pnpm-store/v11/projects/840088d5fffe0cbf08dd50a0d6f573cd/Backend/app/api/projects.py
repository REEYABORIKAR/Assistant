...
from app.schemas.project import ProjectCreate, ProjectUpdate, ProjectResponse, WorkspaceResponse
...
def list_projects(db: SessionDep, current_user: CurrentUser, search: Optional[str] = None):
...
def delete_project(project_id: str, db: SessionDep, current_user: CurrentUser):
    project = get_project_or_404(db, project_id, current_user.id)
    db.delete(project)
    db.commit()
    return None
