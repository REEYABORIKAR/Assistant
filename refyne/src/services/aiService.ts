import type { GenerateResponse, SearchResponse, DocumentGenerationResponse, SupervisorChatResponse } from '../types/models';
import { api } from './api';
...
export const aiService = { 
  ...
  supervisorChat: (projectId: string, conversationId: string, userMessage: string, options?: { action?: string; document_ids?: string[] }) => api<SupervisorChatResponse>('/api/supervisor/chat', { method: 'POST', body: JSON.stringify({ project_id: projectId, conversation_id: conversationId, user_message: userMessage, ...(options?.action ? { action: options.action } : {}), ...(options?.document_ids?.length ? { document_ids: options.document_ids } : {}) }) }),
};